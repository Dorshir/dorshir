...
typedef enum Stack_Result {
    STACK_SUCCESS,
    STACK_UNITIALIZED_ERROR,            /**< Uninitialized stack error */
    STACK_ALLOCATION_ERROR,             /**< Memory allocation error */
    STACK_UNDERFLOW_ERROR,              /**< Empty stack error */
    STACK_OVERFLOW_ERROR,               /**< Stack overflow error */
 ...
 * @retval STACK_UNITIALIZED_ERROR on NULL stack
 * @retval STACK_ALLOCATION_ERROR on memory error
 * @retval STACK_OVERFLOW_ERROR on fixed size stack overflow
 */
 ...
 * @retval STACK_SUCCESS on success 
 * @retval STACK_UNITIALIZED_ERROR on NULL stack
 * @retval STACK_UNDERFLOW_ERROR if stack is empty
 * @warning _pValue can't be null. this will be assertion violation
 */
 ...
 * @param[out] _pValue - pointer to variable that will receive top item value
 * @return success or error code 
 * @retval STACK_SUCCESS on success 
 * @retval STACK_UNITIALIZED_ERROR on NULL stack
 * @retval STACK_IS_EMPTY if stack is empty
 * @warning _pValue can't be null. this will be assertion violation
 */
 ...
 * @return TRUE (1) if empty or ERROR, FALSE(0) if not empty
 */
 ...
 *          if _action returns zero for an element the iteration will stop  
 ...
 * @return number of times the user function was invoked
 */
